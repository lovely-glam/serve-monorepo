...
public interface LoginMethodRepository extends BaseRepository<LoginMethod, BigDecimal> {
    ...
    @Query(value = "SELECT lm FROM login_methods lm WHERE lm.externalId = :externalId AND lm.loginType = :loginMethodType")
    Optional<LoginMethod> findLoginMethodByExternalIdAndLoginMethodType(@Param(value = "externalId") String externalId, @Param(value = "loginMethodType") LoginMethodType loginMethodType);
}
